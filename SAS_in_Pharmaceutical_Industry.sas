...
/************************************************************/
/*****Chapter5 Creating Tables and Listings******/

*using PROC TABULATE to create a summary of demographics*;
...
proc format;
	...
	value race
		...
		3 = "Other*";
run;
**** DEFINE OPTIONS FOR ASCII TEXT OUTPUT__formchar;
...
*PROC TABULATE is an excellent tool for producing quick descriptive statistics on data,
but it does not meet the typical needs of generating clinical trial tables, for several
reasons:
...
*using PROC REPORT to create a summary of Demographics*;
**** INPUT SAMPLE DEMOGRAPHICS DATA;
data demog;
label subjid = "Subject Number"
	trt = "Treatment"
...
for several reasons:
1. It produces only columnar-style reports and not the matrix-style reports required by
clinical trial reporting.
2. It does not provide p-values beyond a simple t-test.
3. “Page X of N” pagination is not available.
4. Missing values are handled in an “all or nothing” way, so if you exclude missing
values for one variable, you end up excluding all data for that record for the other
variables in the summary. Even if missing values are included with the MISSING
option, the N statistic is not calculated as you might expect for missing variables.
5. It does not place population counts in the column heading;
...
*The main difference btw medication tables and adverse events summary is 
that most often medication tables do not have a high summary level, such 
as the body system found in adverse event summaries;
...
**** VARIABLE, WHICH IS USED TO CREATE BREAK LINE IN THE REPORT.
...
**********************************;
*A laboratory shift table is a tabular display that can show you 
how a population’s laboratory data change, or “shift,” over time;
**** INPUT SAMPLE TREATMENT DATA;
data treat;
label subjid = "Subject Number"
		trtcd = "Treatment";
...
************************************************
*****Kaplan-Meier Survival Estimates Tables*****
************************************************
...
*always do your reporting in SAS with simple ASCII text. It is usually 
cheaper to produce and more flexible with page layout than any other 
output destination;

*sending output to ODS RTF destination;
ods rtf file = “demog_table.rtf”;
	proc report
		data = demog;
	etc.
	run;
...
*converting a single SAS output to PDF;
ods pdf file = “demog_table.pdf”;
	proc report
		data = demog;
	etc.
	run;
...
footnote1
"Created by %sysget(PWD)/%sysfunc(scan(&SYSPROCESSNAME,2)).sas
generated on &sysdate9 &systime" ;

*Macro-based reporting systems
Here is a small list of common reporting tasks that you can
look toward generalizing with the SAS macro language:
